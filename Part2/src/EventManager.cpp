...
void EventManager::update()
{
	if(m_isActive)
	{
		m_io = ImGui::GetIO();
		m_mouseWheel = 0;
		m_mouseButtonsLastFrame[0] = m_mouseButtons[0];
		m_mouseButtonsLastFrame[1] = m_mouseButtons[1];
		m_mouseButtonsLastFrame[2] = m_mouseButtons[2];
...
bool EventManager::getMouseButtonDown(int button_number) const
{
	// The button is considered down if it is down now, but was NOT down last frame.
	return (m_mouseButtons[button_number] == true && m_mouseButtonsLastFrame[button_number] == false);
}

bool EventManager::getMouseButtonUp(int button_number) const
{
	// The button is considered up if it is up now, but was NOT up last frame.
	return (m_mouseButtons[button_number] == false && m_mouseButtonsLastFrame[button_number] == true);
}
