...
void PlayScene::update()
{
	...
	// Handle ball bounds
	if (m_pBall->getTransform()->position.x < 0.0f)
	{
		m_pBall->getTransform()->position.x = 0.0f;
		m_pBall->getRigidBody()->velocity.x = m_pBall->getRigidBody()->velocity.x * m_pBall->bounciness;
	}
	else if (m_pBall->getTransform()->position.x > 800.0f)
	{
		m_pBall->getTransform()->position.x = 800.0f;
		m_pBall->getRigidBody()->velocity.x = m_pBall->getRigidBody()->velocity.x * m_pBall->bounciness;
		std::cout << "REVERSING BALL BOUNCE ON X\n";
	}
	if (m_pBall->getTransform()->position.y < 0.0f)
	{
		m_pBall->getTransform()->position.y = 0.0f;
		m_pBall->getRigidBody()->velocity.y = m_pBall->getRigidBody()->velocity.y * m_pBall->bounciness;

	}
	else if (m_pBall->getTransform()->position.y > 600.0f)
	{
		m_pBall->getTransform()->position.y = 600.0f;
		m_pBall->getRigidBody()->velocity.y = m_pBall->getRigidBody()->velocity.y * m_pBall->bounciness;
	}
